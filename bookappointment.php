...
$result = mysql_query("SELECT * FROM doctor WHERE id = '".$docId."'")
or die(mysql_error());  


$result_reasons = mysql_query("SELECT * FROM specialtyreasons, doctor, doctorspecialty, visitreason WHERE doctor.id = '".$docId."' AND specialtyreasons.specialty_id = doctorspecialty.specialty_id
	...
	<form id="myForm"  name="confirmation-form" method="post" action="confirmation.php">
            ...
            ?>
				...
				<select id="reason" name="reason" class="validate[required]">
					...
					<?php					
						while($row_reasons = mysql_fetch_array( $result_reasons )){
